<template>
  <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-slate-800">{{ loading ? '—' : stats.totalVendors }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-slate-800">{{ loading ? '—' : stats.activeVendors }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-slate-800">{{ loading ? '—' : stats.pendingVendors }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-slate-800">{{ loading ? '—' : stats.totalOrders }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-slate-800">{{ loading ? '—' : stats.totalUsers }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-slate-800">{{ loading ? '—' : formatCurrency(stats.totalRevenue) }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-slate-800">{{ loading ? '—' : stats.totalProducts }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-slate-800">{{ loading ? '—' : stats.pendingOrders }}</p>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-8">
      ...
      <div class="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
        ...
        <div v-if="loading" class="flex items-center justify-center h-32 text-slate-400 text-sm">Loading...</div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-5">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
        ...
        <div v-if="loading" class="text-sm text-slate-400 text-center py-6">Loading...</div>
...
async function fetchDashboardData() {
  loading.value = true
  try {
    const [vendorRes, productRes, profileRes, orderRes] = await Promise.all([
    ...
    // Orders
    if (!orderRes.error) {
      const allOrdersRes = await supabase.from('orders').select('id,total_amount,status,created_at', { count: 'exact' })
      ...
      recentOrders.value = (orderRes.data || []).map(o => ({ ...o, buyer_name: null }))
    }
  } catch (e) {
